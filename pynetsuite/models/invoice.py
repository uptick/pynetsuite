...
class Invoice(BaseModel):
    ...
    start_date: Optional[date] = Field(default=None, description="If you use the Advanced Billing feature, enter the date for the first invoice to be created. If an invoice is created from a sales order that includes a Start Date and End Date, those dates autofill on the invoice when the order is billed.", alias="startDate")
    oppcreatedfromtitle: Optional[StrictStr] = None
    ...
    subtotal: Optional[Union[StrictFloat, StrictInt]] = Field(default=None, description="This field shows the total of the items on this invoice. This is the total before any discounts, shipping cost, handling cost or tax is added to the order.")
    ...
    amount_paid: Optional[Union[StrictFloat, StrictInt]] = Field(default=None, description="Displays the total amount paid for this invoice.", alias="amountPaid")
    ...
    as_of_date: Optional[date] = Field(default=None, description="The As-of Date field determines the list of charges added to the transaction, but it does not impact the general ledger or the invoice due date. For example, you may have an as-of date that is after the invoice date when you want to capture charges that will post after the invoice date.", alias="asOfDate")
    ...
    sales_effective_date: Optional[date] = Field(default=None, description="You can change the sales effective date for this transaction. The sales effective date determines which commission plan and historical sales team this transaction applies to.", alias="salesEffectiveDate")
    custom_form: Optional[InvoiceCustomForm] = Field(default=None, alias="customForm")
    ...
    ship_address: Optional[StrictStr] = Field(default=None, description="The default shipping address autofills this field from the customer&apos;s record. To enter a different address: * Select another address in the Ship To Select field. * Select New in the Ship To Select field to enter a new shipping address to be used for this transaction and saved with the associated entity record. * Select Custom in the Ship To Select field to enter a new shipping address to be used for this transaction only (and not saved with the associated entity record). * Click the Edit icon for the Ship To Select field to edit an existing shipping address.", alias="shipAddress")
    ...
    to_be_printed: Optional[StrictBool] = Field(default=None, description="When processing individual invoices: Check the To Be Printed box if you want to print this invoice. When bulk processing invoices: * Select Yes if you want to save this in a queue of invoices to print. * Select No if you do not want to print this form. * Select Respect Customer Preference to send the form based on the default preference on the customer record. Set the preference on the Info subtab of a customer record in the Send Transactions Via field.", alias="toBePrinted")
    ...
    to_be_emailed: Optional[StrictBool] = Field(default=None, description="When processing individual invoices: * Check the To Be Emailed box if you want to email this invoice. Then enter the email address in the space to the right of the check box. * You can enter multiple email addresses by separating them with a semicolon. It is important to separate the addresses with only a semicolon, and do not include any spaces. When bulk processing invoices: In the To Be Emailed field: * Select Yes if you want to email this form. * Select No if you do not want to email this form. * Select Respect Customer Preference to send the form based on the default preference on the customer record. Set the preference on the Info subtab of a customer record in the Send Transactions Via field.", alias="toBeEmailed")
    ...
    discount_date: Optional[date] = Field(default=None, description="The date listed here is the date your customer must pay this invoice by in order to receive a discount. The discount your customer receives in based on the terms you have set for the customer.", alias="discountDate")
    custbody_vl_req_date_required: Optional[date] = None
    custbody_ff_sc_trans_ita_apy_code: Optional[StrictStr] = None
    tracking_numbers: Optional[StrictStr] = Field(default=None, description="Enter the shipment tracking number from the shipping company. If a tracking number is entered in the exact format used by UPS or FedEx and the transaction is saved, the tracking number will show on the transaction as a link to the UPS or FedEx website. You can click the link to check the status of the shipment.", alias="trackingNumbers")
    custbody_bb1_roc_label: Optional[StrictStr] = None
    fob: Optional[StrictStr] = Field(default=None, description="Enter the location where the customers technically acquire ownership. For example, if your customers own goods as soon as they ship and pay the shipping costs from your location to theirs, you might enter Our warehouse.")
    ...
    to_be_faxed: Optional[StrictBool] = Field(default=None, description="When processing individual invoices: * Check the To Be Faxed box if you want to fax this invoice. Then enter the fax number in the box to the right of the check box if it does not already appear. When bulk processing invoices: In the To Be Faxed field: * Select Yes if you want to fax this form. * Select No if you do not want to fax this form. * Select Respect Customer Preference to send the form based on the default preference on the customer record. Set the preference on the Info subtab of a customer record in the Send Transactions Via field. To fax NetSuite forms, an administrator must first set up fax service at Setup &gt; Company &gt; Printing, Fax and Email Preferences.", alias="toBeFaxed")
    ...
    ship_date: Optional[date] = Field(default=None, description="Type or pick a shipping date for this invoice.", alias="shipDate")
    ...
    previous_opportunity: Optional[StrictStr] = Field(default=None, alias="previousOpportunity")
    custbody_ff_sc_clickdata: Optional[StrictStr] = None
    message: Optional[StrictStr] = Field(default=None, description="The text of the customer message you select in the Select Message field appears here. You can edit the message or enter a new customer message for only this transaction.")
    ...
    amount_remaining_total_box: Optional[Union[StrictFloat, StrictInt]] = Field(default=None, description="The total unpaid amount on this invoice is shown here.", alias="amountRemainingTotalBox")
    ...
    due_date: Optional[date] = Field(default=None, description="Type or pick the due date for this invoice. If you do not assign a due date, the due date defaults to the date in the Date field. In addition, if you do not assign a due date, this invoice will appear on aging reports.", alias="dueDate")
    ...
    tran_date: Optional[date] = Field(default=None, description="NetSuite inserts today&apos;s date for the posting date of this invoice. You can type or pick another date. What you enter here determines the date range in which this transaction appears on the Accounts Receivable Register. You can select or enter custom date ranges on registers.", alias="tranDate")
    custbody_bb1_print_bom: Optional[StrictStr] = None
    memo: Optional[StrictStr] = Field(default=None, description="Enter an optional memo to describe this invoice so you can easily distinguish it from others for this customer. For example: Suite A installation or Suite B installation. It will appear on reports such as the 2-line Accounts Receivable register, which your customers can see if you give them permission to log in and view their transaction history.")
    ...
    bill_address: Optional[StrictStr] = Field(default=None, description="The default billing address autofills this field from the customer&apos;s record. To enter a different address: * Select another address in the Bill To Select field. * Select New in the Bill To Select field to enter a new billing address to be used for this transaction and saved with the associated entity record. * Select Custom in the Bill To Select field to enter a new billing address to be used for this transaction only (and not saved with the associated entity record). * Click the Edit icon for the Bill To Select field to modify an existing billing address.", alias="billAddress")
    ...
    end_date: Optional[date] = Field(default=None, description="Type or pick the end date for this invoice. If an invoice is created from a sales order that includes a Start Date and End Date, those dates autofill on the invoice when the order is billed.", alias="endDate")
    ...
    discount_amount: Optional[Union[StrictFloat, StrictInt]] = Field(default=None, description="The amount listed here is the discount your customer will receive if he or she pays this invoice according to the terms you have set.", alias="discountAmount")
    custbody_nx_idempotency_key: Optional[StrictStr] = None
    custbody_ff_sc_tax_scheme_id: Optional[StrictStr] = None
    linked_tracking_numbers: Optional[StrictStr] = Field(default=None, alias="linkedTrackingNumbers")
    which_charges_to_add: Optional[CreditMemoWhichChargesToAdd] = Field(default=None, alias="whichChargesToAdd")
    amount_remaining: Optional[Union[StrictFloat, StrictInt]] = Field(default=None, description="Displays the total amount due for this invoice.", alias="amountRemaining")
    ...
    oppcreatedfrom: Optional[StrictStr] = None
    custbody_eff_nsp2p_supplier_notes: Optional[StrictStr] = None
    for_invoice_grouping: Optional[StrictBool] = Field(default=None, description="Check the For Invoice Grouping box to group invoices based on common criteria. For more information, see Invoice Groups Overview.", alias="forInvoiceGrouping")
    ...
    discount_total: Optional[Union[StrictFloat, StrictInt]] = Field(default=None, description="NetSuite enters the amount discounted on this invoice. If this discount item is taxable, the discount is applied before taxes. If it is not taxable, the discount is applied after taxes.", alias="discountTotal")
    ...
    other_ref_num: Optional[StrictStr] = Field(default=None, description="To make it easy for your customers to locate this invoice in their own records, enter their purchase order number here.", alias="otherRefNum")
    custbody_vl_total_re_invoice_value: Optional[Union[StrictFloat, StrictInt]] = None
    total: Optional[Union[StrictFloat, StrictInt]] = Field(default=None, description="NetSuite computes the total of line items, tax and shipping costs, and then enters it here.")
    ...
    tran_id: Optional[StrictStr] = Field(default=None, description="NetSuite increases the largest invoice number by one. You can type another number. The next invoice number will revert to the standard pattern.", alias="tranId")
    ...
    exp_cost: Optional[InvoiceExpCostCollection] = Field(default=None, alias="expCost")
    ...
    custbody_zc_ap_bank_acct_gl_acct: Optional[NsResource] = None
    ...
    grouped_to: Optional[NsResource] = Field(default=None, alias="groupedTo")
    sales_rep: Optional[VendorBillItemElementCustomer] = Field(default=None, alias="salesRep")
    time: Optional[InvoiceTimeCollection] = None
    ...
    shipping_address: Optional[InvoiceShippingAddress] = Field(default=None, alias="shippingAddress")
    item: Optional[InvoiceItemCollection] = None
    ...
    account: Optional[NsResource] = None
    ...
    item_cost: Optional[InvoiceItemCostCollection] = Field(default=None, alias="itemCost")
    custbody_4601_entitydefaultwitaxcode: Optional[NsResource] = None
    custbody_nx_task: Optional[NsResource] = None
    accounting_book_detail: Optional[InvoiceAccountingBookDetailCollection] = Field(default=None, alias="accountingBookDetail")
    ...
    opportunity: Optional[NsResource] = None
    ...
    billing_address: Optional[InvoiceBillingAddress] = Field(default=None, alias="billingAddress")
    __properties: ClassVar[List[str]] = ["custbody_bb1_vl_rejection_reason", "custbody_terms_dry_riser_maintenance", "custbody_bb1_sc_nodates", "startDate", "oppcreatedfromtitle", "custbody_terms_lighting_maintenance", "custbody_bb1_quote_released", "estimate", "custbody_terms_active_payment_install", "custbody_terms_maintenance_assumptions", "custbody_ph4014_wtax_applied", "custbody_bb1_open_cr_value", "custbody_terms_provisional_costs", "custbody_4601_total_amt", "custbody_terms_payment_for_works", "subtotal", "billAddr1", "totalCostEstimate", "amountPaid", "billAddr2", "billAddr3", "custbody_ph4014_wtax_wamt", "custbody_terms_compartment_install", "custbody_link_lsa", "asOfDate", "createdDate", "custbody_terms_painting_and_decorating", "custbody_nx_customer_signature", "custbody_ph4014_wtax_code", "custbodybillreferencenumber", "custbody_terms_payment_healthchecks", "custbody_bb1_hubspot_record_id", "custbody_adjustment_journal", "custbody_vl_scheduled_auto_approval", "custbody_client_project_manager_email", "custbody_terms_extinguish_maintenance", "email", "custbody_link_name_lsa", "canHaveStackable", "custbody_bb1_vl_action_reject_field", "custbody_zc_ap_gl_acct_balance", "custbody_country_of_origin", "custbody_terms_fd_hc_assumptions", "custbody_unique_asset_code", "custbody_vl_variation_labour_budget", "custbody_zc_3wm_translations", "custbody_vl_variation_prelim_budget", "prevDate", "custbody_itr_nexus", "salesEffectiveDate", "customForm", "custbody_terms_damper_maintenance", "custbody_report_timestamp", "custbody_bb1_cc_note", "custbody_msl_estimated_weeks", "custbody_vl_req_contact_name", "prevRep", "custbody_terms_sprinkler_maintenance", "custbody_ff_sc_endpoint_scheme_id", "shipAddress", "custbody_ph4014_wtax_bamt", "custbody_erpff_p2p_auto_send_document", "externalId", "source", "custbody_wtax_base_url", "custbody_ff_sc_endpoint_op_id", "toBePrinted", "custbody_vl_variation_material_budget", "billPhone", "custbodyhs_integration_embed", "billAttention", "custbody_terms_contingency", "custbodycus_ven_sales_order_department", "custbody_test_terms_pdf", "toBeEmailed", "custbody_msl_commission_generated", "custbody_bb1_link_to_sharepoint", "custbody_zc_ap_main_config_data", "discountDate", "custbody_vl_req_date_required", "custbody_ff_sc_trans_ita_apy_code", "trackingNumbers", "custbody_bb1_roc_label", "fob", "custbody_ph4014_wtax_reversal_flag", "custbody_vl_temp", "custbodyinvoiceonhold", "custbody_ff_sc_field_block_mapping", "custbody_vl_invoice_group_no", "custbody_total_quote_gp_percent", "exchangeRate", "custbody_itr_doc_number", "custbody_vl_req_contact_number", "custbody_zc_ai_field_application_data", "custbody_bb1_val_credited", "custbody_bb1_custom_address", "originator", "custbody_bb1_legacytrannumber", "billZip", "toBeFaxed", "custbody_bb1_release_check", "custbody_bb1_cr_supportingfile", "custbody_vl_original_site_mgmt_budget", "custbody_4601_entitytype", "custbody_ff_sc_e_inv_pdf", "custbody_bb1_save_as_draft", "custbody_terms_hc_assumptions", "custbody_vl_original_labour_budget", "shipDate", "shippingAddress_text", "custbody_vl_bill_pymt_je_cr_data", "custbody_terms_accreditations", "custbody_bb1_vl_action_approve_field", "previousOpportunity", "custbody_ff_sc_clickdata", "message", "custbody_terms_compartment_maintenance", "custbody_vl_original_total_budget", "custbody_terms_suppresion_maintenance", "amountRemainingTotalBox", "billAddressee", "custbody_bb1_sc_liquid_damages", "dueDate", "custbody_erpff_p2p_basware_send_note", "custbody_intercompany_review", "billCity", "custbody_terms_fire_alarm_install", "custbody_terms_compartmentation_hc", "tranDate", "custbody_bb1_print_bom", "memo", "billCountry", "custbody_eff_nsp2p_buyerref", "custbody_vl_original_material_budget", "custbody_date_of_taxable_supply", "id", "custbody_vl_export_quote", "custbody_vl_variation_total_budget", "billAddress", "custbody_vl_variation_site_manage_budg", "estGrossProfit", "custbody_vl_orig_preliminaries_budget", "endDate", "custbody_eff_nsp2p_paymentref", "custbody_cash_register", "custbody_bb1_custom_notes", "custbody_terms_fd_healthcheck", "custbody_subconnotes", "status", "custbody_update", "custbody_bb1_building_address", "shipIsResidential", "estGrossProfitPercent", "custbody_zc_3wm_acceptance_reason", "custbody_msl_qs_invoice", "tranIsVsoeBundle", "custbody_application_date", "custbody_terms_compartment_support", "custbody_ph4014_wtax_rate", "shipOverride", "discountAmount", "custbody_nx_idempotency_key", "custbody_ff_sc_tax_scheme_id", "linkedTrackingNumbers", "whichChargesToAdd", "amountRemaining", "custbody_eff_nsp2p_serialnumbers", "custbody_terms_works_assumptions", "discountRate", "billState", "custbody_4601_transactions", "custbody_terms_hc_accreditations", "custbody_terms_smoke_vent_maintenance", "custbody_terms_fd_installation", "custbody_erpff_p2p_document_sent", "custbody_zc_ap_bank_integ_id", "custbody_is_a_variation", "custbody_auto_send_document_nonstore", "custbody_estimated_gp", "custbody_zc_ap_bank_acct_balance", "oppcreatedfrom", "custbody_eff_nsp2p_supplier_notes", "forInvoiceGrouping", "billOverride", "custbody_erpff_p2p_customer_reference", "billingAddress_text", "discountTotal", "custbody_4110_customregnum", "custbodyinterlegacytrannumber", "custbody_vl_group_invoice", "custbody_ff_sc_deliverydate", "custbody_4601_wtax_withheld", "custbody_bb1_vendor_phone", "custbody_ff_sc_endpoint_scheme", "custbody_zc_ap_subsidiary_config_data", "custbody_bb1_sc_completiondate", "otherRefNum", "custbody_vl_total_re_invoice_value", "total", "custbody_ff_sc_bank_details_einvoices", "custbody_terms_fire_alarm_maintenance", "custbody_ff_sc_tax_id", "custbody_4601_appliesto", "custbody_msl_site_address_hidden", "custbody_nondeductible_processed", "custbody_ff_sc_endpoint_id", "custbody_ph4014_wtax_cpay_pacct", "custbody_date_lsa", "tranId", "custbody_counterparty_vat", "custbody_refno_originvoice", "lastModifiedDate", "intercoStatus", "custbody_msl_site_address", "custbody_ff_sc_b2pmodel", "custbody_bb1_sc_startdate", "custbody_vl_req_extra_comments", "custbody_bb1_credit_created", "refName", "links", "createdFrom", "custbody_requestor_header", "job", "custbody_ff_sc_trans_ita_pay_term", "messageSel", "shipAddressList", "postingPeriod", "custbody_transaction_region", "custbody_payment_type", "custbody_ff_sc_bank_details", "custbody_msl_salesrep", "custbody_alternate_site", "class", "custbody_ff_sc_bank_details_sec", "cseg_accreditations", "billAddressList", "custbody_bb1_vl_project_owner", "intercoTransaction", "partner", "custbody_subcontractor", "custbody_tc_relatedtask", "custbody_nx_case", "custbody_regime_code", "expCost", "currency", "custbody_ff_sc_po_validat_rule_applied", "custbody_zc_ap_pay_from", "entity", "custbody_nx_customer", "custbody_vl_req_urgency_tier", "custbody_bb1_vl_current_approval", "custbody_zc_ap_bank_acct_gl_acct", "custbody_4601_doc_ref_id", "custbody_suppliessupplier", "location", "discountItem", "custbody_projectbuyer", "custbodyprojectqsheader", "custbody_bb1_suiteprojects_project", "custbody_regime_code_of_supply", "shipMethod", "custbody_vl_req_your_name", "custbody_notc", "custbody_vl_building_list", "custbody_4601_pymnt_ref_id", "custbody_cs_ventro_estimator", "custbody_nondeductible_ref_genjrnl", "custbody_vl_next_approver", "sourceWebSite", "department", "custbody_vl_new_invoice_number", "custbodypmscheduler", "groupedTo", "salesRep", "time", "custbody_eff_nsp2p_xml2nstrans", "custbody_bb1_credit_reason", "terms", "custbody_vl_linked_transaction", "custbody_nx_projecttask", "bulkProcSubmission", "custbody_bb1_created_by", "custbody_vl_req_request_type", "custbody_bb1_dispute_reason", "shippingAddress", "item", "custbody_ordertype", "custbody_nexus_notc", "custbody_delivery_terms", "subsidiary", "custbody_bb1_vl_project", "custbody_nx_case_type", "custbody_zc_ap_pay_to", "account", "cseg_paactivitycode", "custbody_bb1_current_cr_approver", "custbody_nx_asset", "custbody_vl_payment_certificate", "itemCost", "custbody_4601_entitydefaultwitaxcode", "custbody_nx_task", "accountingBookDetail", "custbody_nondeductible_ref_tran", "custbody_cs_linked_project", "custbody_bb1_building", "custbody_ph4185_bstyle", "custbody_billingcontact", "opportunity", "custbody_bb1_credit_requestor", "custbody_mode_of_transport", "custbody_project_framework", "custbody_bb1_market", "billingAddress"]
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * OpenAPI `readOnly` fields are excluded.
        ...
        # override the default output from pydantic by calling `to_dict()` of exp_cost
        if self.exp_cost:
            _dict['expCost'] = self.exp_cost.to_dict()
        ...
        # override the default output from pydantic by calling `to_dict()` of grouped_to
        if self.grouped_to:
            _dict['groupedTo'] = self.grouped_to.to_dict()
        ...
        # override the default output from pydantic by calling `to_dict()` of item_cost
        if self.item_cost:
            _dict['itemCost'] = self.item_cost.to_dict()
        ...
        # set to None if oppcreatedfromtitle (nullable) is None
        # and model_fields_set contains the field
        if self.oppcreatedfromtitle is None and "oppcreatedfromtitle" in self.model_fields_set:
            _dict['oppcreatedfromtitle'] = None
        ...
        # set to None if fob (nullable) is None
        # and model_fields_set contains the field
        if self.fob is None and "fob" in self.model_fields_set:
            _dict['fob'] = None
        ...
        # set to None if previous_opportunity (nullable) is None
        # and model_fields_set contains the field
        if self.previous_opportunity is None and "previous_opportunity" in self.model_fields_set:
            _dict['previousOpportunity'] = None
        ...
        # set to None if linked_tracking_numbers (nullable) is None
        # and model_fields_set contains the field
        if self.linked_tracking_numbers is None and "linked_tracking_numbers" in self.model_fields_set:
            _dict['linkedTrackingNumbers'] = None
        ...
        # set to None if oppcreatedfrom (nullable) is None
        # and model_fields_set contains the field
        if self.oppcreatedfrom is None and "oppcreatedfrom" in self.model_fields_set:
            _dict['oppcreatedfrom'] = None
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            ...
            "oppcreatedfromtitle": obj.get("oppcreatedfromtitle"),
            ...
            "customForm": InvoiceCustomForm.from_dict(obj["customForm"]) if obj.get("customForm") is not None else None,
            ...
            "previousOpportunity": obj.get("previousOpportunity"),
            ...
            "status": InvoiceStatus.from_dict(obj["status"]) if obj.get("status") is not None else None,
            ...
            "linkedTrackingNumbers": obj.get("linkedTrackingNumbers"),
            ...
            "oppcreatedfrom": obj.get("oppcreatedfrom"),
            "custbody_eff_nsp2p_supplier_notes": obj.get("custbody_eff_nsp2p_supplier_notes"),
            "forInvoiceGrouping": obj.get("forInvoiceGrouping"),
            ...
            "expCost": InvoiceExpCostCollection.from_dict(obj["expCost"]) if obj.get("expCost") is not None else None,
            ...
            "custbody_zc_ap_bank_acct_gl_acct": NsResource.from_dict(obj["custbody_zc_ap_bank_acct_gl_acct"]) if obj.get("custbody_zc_ap_bank_acct_gl_acct") is not None else None,
            ...
            "groupedTo": NsResource.from_dict(obj["groupedTo"]) if obj.get("groupedTo") is not None else None,
            "salesRep": VendorBillItemElementCustomer.from_dict(obj["salesRep"]) if obj.get("salesRep") is not None else None,
            "time": InvoiceTimeCollection.from_dict(obj["time"]) if obj.get("time") is not None else None,
            ...
            "shippingAddress": InvoiceShippingAddress.from_dict(obj["shippingAddress"]) if obj.get("shippingAddress") is not None else None,
            "item": InvoiceItemCollection.from_dict(obj["item"]) if obj.get("item") is not None else None,
            ...
            "account": NsResource.from_dict(obj["account"]) if obj.get("account") is not None else None,
            ...
            "itemCost": InvoiceItemCostCollection.from_dict(obj["itemCost"]) if obj.get("itemCost") is not None else None,
            ...
            "accountingBookDetail": InvoiceAccountingBookDetailCollection.from_dict(obj["accountingBookDetail"]) if obj.get("accountingBookDetail") is not None else None,
            ...
            "opportunity": NsResource.from_dict(obj["opportunity"]) if obj.get("opportunity") is not None else None,
            ...
            "billingAddress": InvoiceBillingAddress.from_dict(obj["billingAddress"]) if obj.get("billingAddress") is not None else None
        })
        return _obj
